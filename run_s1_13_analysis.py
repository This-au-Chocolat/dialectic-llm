...
def analyze_s1_13(baseline_path, tas_path):
    ...
    common_problem_ids = pd.merge(baseline_df[['problem_id']],
                                  tas_df[['problem_id']],
                                  on='problem_id',
                                  how='inner')['problem_id']

    if common_problem_ids.empty:
        print("No common problem_ids found between baseline and TAS datasets. Cannot perform McNemar's test.")
        return

    baseline_common = baseline_df[baseline_df['problem_id'].isin(common_problem_ids)].set_index('problem_id')
    ...
    aligned_df = pd.merge(baseline_common, tas_common, on='problem_id', suffixes=('_baseline', '_tas'))
    ...
    b = aligned_df[(aligned_df['is_correct_baseline'] == False) & (aligned_df['is_correct_tas'] == True)].shape[0]
    c = aligned_df[(aligned_df['is_correct_baseline'] == True) & (aligned_df['is_correct_tas'] == False)].shape[0]
    ...
    if (b + c) < 2: # At least two discordant pairs are recommended for McNemar's.
        print(f"Not enough discordant pairs ({b+c}) to perform McNemar's test meaningfully. Need at least 2.")
        ...
        n00 = aligned_df[(aligned_df['is_correct_baseline'] == True) & (aligned_df['is_correct_tas'] == True)].shape[0]
        n01 = c # Baseline correct, TAS incorrect
        n10 = b # Baseline incorrect, TAS correct
        n11 = aligned_df[(aligned_df['is_correct_baseline'] == False) & (aligned_df['is_correct_tas'] == False)].shape[0]

        table = [[n00, n01],
                 [n10, n11]]
        
        print(f"McNemar Contingency Table:\n{pd.DataFrame(table, index=['Baseline Correct', 'Baseline Incorrect'], columns=['TAS Correct', 'TAS Incorrect'])}\n")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: python run_s1_13_analysis.py <path_to_baseline.parquet> <path_to_tas.parquet>")
        sys.exit(1)
    
    baseline_file = sys.argv[1]
    tas_file = sys.argv[2]
    
    analyze_s1_13(baseline_file, tas_file)
