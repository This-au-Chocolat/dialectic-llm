...
class TASConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        env_overrides = {
            'thesis': {
                'temperature': float(os.getenv('TAS_THESIS_TEMPERATURE', 
                                            config.get('thesis', {}).get('temperature', 0.7)))
            },
            'antithesis': {
                'temperature': float(os.getenv('TAS_ANTITHESIS_TEMPERATURE',
                                             config.get('antithesis', {}).get('temperature', 0.5)))
            },
            'synthesis': {
                'temperature': float(os.getenv('TAS_SYNTHESIS_TEMPERATURE',
                                             config.get('synthesis', {}).get('temperature', 0.2)))
            },
            'tas': {
                'k': int(os.getenv('TAS_K_VALUE', 
                                 config.get('tas', {}).get('k', 1)))
            },
            'models': {
                'primary': os.getenv('TAS_DEFAULT_MODEL', 
                                   config.get('models', {}).get('primary', 'gpt-4')),
                'fallback': os.getenv('TAS_FALLBACK_MODEL',
                                    config.get('models', {}).get('fallback', 'gpt-3.5-turbo'))
            },
            'limits': {
                'max_tokens_per_phase': int(os.getenv('TAS_MAX_TOKENS_PER_PHASE',
                                                    config.get('limits', {}).get('max_tokens_per_phase', 2000))),
                'total_session_limit': int(os.getenv('TAS_TOTAL_TOKEN_LIMIT',
                                                   config.get('limits', {}).get('total_session_limit', 6000))),
                'timeout_seconds': int(os.getenv('TAS_REQUEST_TIMEOUT',
                                               config.get('limits', {}).get('timeout_seconds', 30))),
                'max_retries': int(os.getenv('TAS_MAX_RETRIES',
                                           config.get('limits', {}).get('max_retries', 3)))
            },
            'logging': {
